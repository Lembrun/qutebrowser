...
def _parse_search_term(s: str) -> Tuple[Union[Optional[str],Optional[List]], Union[Optional[str],Optional[List]]]:
    ...
    if len(split) == 2 and len(split_engines) == 1:
        if split[0] in config.val.url.searchengines:
            engine: Optional[Union[str, List]] = split[0]
            term: Optional[Union[str, List]] = split[1]
        else:
            engine = None
            term = s
...
def _get_search_url(txt: str) -> Union[QUrl, List[QUrl]]:
